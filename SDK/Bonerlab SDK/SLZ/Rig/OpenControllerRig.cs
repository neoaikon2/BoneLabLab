...
namespace SLZ.Rig
{
	public class OpenControllerRig : ControllerRig
	{
		...
		private void ProcessTrackedBlend(Transform output, SimpleTransform target, bool tracked, TrackedState state, SimpleTransform offset, float blend, float velocity, float time)
		{
		}

		private ValueTuple<SimpleTransform, float, float, float> SnapshotOffset(SimpleTransform target, Transform output)
		{
			return default(ValueTuple<SimpleTransform, float, float, float>);
		}

		private void BlendOffset(Transform output, SimpleTransform target, float time, SimpleTransform offset, float blend, float velocity)
		{
		}
		...
		private ValueTuple<float, float, bool> ProcessSecondaryThumbstick(Vector2 axis, float deltaTime)
		{
			return default(ValueTuple<float, float, bool>);
		}
